  ...
  return (
    <div className="page-container">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6 mb-8">
        {stats.map((stat, i) => (
          <div key={i} className="bg-white p-4 lg:p-6 rounded-xl border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-2 lg:gap-0">
              <div>
                <p className="text-[10px] lg:text-sm text-gray-500 uppercase font-bold tracking-wider">{stat.label}</p>
                <p className={`text-xl lg:text-3xl font-bold mt-1 ${stat.color}`}>{stat.value}</p>
              </div>
              <div className={`${stat.bg} p-2 lg:p-3 rounded-full text-xl lg:text-2xl w-fit`}>
                {stat.icon}
              </div>
            </div>
          </div>
      ...
      <div className="card mb-8">
        <h2 className="text-lg font-bold mb-4 lg:mb-6 text-gray-700">Asset Condition Overview</h2>
        <div className="h-[250px] sm:h-[300px] w-full">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={healthData} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
            ...
            <div className="grid grid-cols-2 gap-3">
              {shortcuts.map((shortcut, i) => (
                <button
                  ...
                  className="flex flex-col items-center justify-center p-4 rounded-xl border border-gray-50 bg-gray-50/50 hover:bg-blue-50 hover:border-blue-100 transition-all group"
                >
                  <span className="text-2xl mb-2 group-hover:scale-110 transition-transform">{shortcut.icon}</span>
                  <span className="text-xs lg:text-sm font-semibold text-gray-600 group-hover:text-blue-600">{shortcut.title}</span>
                </button>
              ))}
            </div>
          </div>
        ...
        <div className="lg:col-span-2 order-1 lg:order-2">
          <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm h-full">
            ...
            <button className="w-full mt-6 py-2 text-blue-600 text-sm font-bold border border-blue-50 rounded-lg hover:bg-blue-50 transition-colors">
              View All Activity
            </button>
          </div>
        </div>
